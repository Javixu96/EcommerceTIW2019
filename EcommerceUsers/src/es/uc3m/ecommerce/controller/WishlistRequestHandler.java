...
public class WishlistRequestHandler implements IHandler{
	...
	public String handleRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(session.getAttribute("user") == null) {
			...
			// Si el usuario todav�a no ha usado el carro, creamos su atributo de sesi�n con los productos y cantidades del carro vac�os
			if(session.getAttribute("wishlistList") == null) {
				wishlistList = new ArrayList<Product>();
				wishlistTotal = 0; 

			// Si el usuario ya ha usado el carro, los atributos de sesi�n ya existen
			} else {
			...
			// ACTION = 2 - A�ADIR PRODUCTO A WISHLIST	
			} else if(action == 2) {
